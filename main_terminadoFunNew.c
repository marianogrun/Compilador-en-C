...
typedef enum {NULO, FIN_DE_ARCHIVO, BEGIN, END, ABREPAREN, CIERRAPAREN, READLN, WRITE, WRITELN, VAR, CADENA, IDENT, NUMERO, PUNTO, COMA, PTOCOMA, MAS,
               CONST,IGUAL,MAYOR, MENOR, PROCEDURE, ASIGNAR, CALL, IF,ELSE,NOT,INC, HALT,SHL,EXIT, DIV, THEN, WHILE,FOR, DO, ODD, DISTINTO, MENORIGUAL, MAYORIGUAL,MENOS, MULTIPLICAR, DIVIDIR } tTerminal;
...
tSimbolo bloque (tSimbolo s, FILE *archivo, tablaDeIdent tabla, int base, byte memoria[], int *tope, int *contadorVariables){
    ...
    if(s.simbolo==CONST){
        ...
        if(s.simbolo==IGUAL||s.simbolo==ASIGNAR) s=aLex(archivo);
...
tSimbolo proposicion (tSimbolo s, FILE *archivo, tablaDeIdent tabla,int base, int posUltimoIdent,byte memoria[],int *tope, int * contadorVariables){
    int p, posPrevCond, posPostCond, origSalto, destSalto, origSaltoElse, destSaltoElse, dist, distancia, posPrevProp;
    ...
    case IDENT:
        ...
        s=aLex(archivo);
        if(s.simbolo==ASIGNAR || s.simbolo==IGUAL)s=aLex(archivo);
    ...
    case WRITELN:
      ...
      if (s.simbolo == PTOCOMA || s.simbolo != ABREPAREN) {         // Si se lo invoca sin parametros, es un salto de linea nomas
            cargarByte(0xE8, memoria, tope);
            cargarInt(0x410-(*tope+4),memoria,tope);  // Salto de linea
        return s;
      }
    ...
    if(s.simbolo==ODD){
        ...
        switch(operador){
         ...
         if (isIfNot) {                                                  // Si es un IF NOT, al darse la condicion true saltea esta instruccion
            cargarByte(0xE9,memoria,tope);                                  // y entra directamente en el E9 siguiente que saltea toda la proposicion
            cargarInt(5,memoria,tope);                                      // Si la condicion es FALSE, entra en este E9 y saltea el E9 siguiente
            }
    }
...
tSimbolo aLex(FILE* fp) {
    ...
    if (c == EOF){
        ...
        if (isalpha(c)) {
            ...
            else if (strcmp(cadenaAux, "DIV") == 0) a.simbolo = DIV;
